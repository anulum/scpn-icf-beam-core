...
bind. Five candidates apply to the configurations this repository owns: `inertial.driver_timing` (event relative),
`inertial.implosion_trajectory` and `inertial.shot_outcome` (noncyclic
features), `inertial.resolved_asymmetry_mode` (derived cyclic), and
`model.synthetic_oscillator_coordinate` (numerical only).

## Decision

1. The capability `diagnostic_clock_semantics` is implemented as a
   declaration model (`src/scpn_icf_beam_core/observability.py`): frozen
...
3. Alignment is enforced structurally: a channel must address an
   ...
   (event-relative to the shot fiducial, derived cyclic to the
   facility monotonic clock, noncyclic to either non-simulation clock,
   numerical-only to the simulation clock).
4. Timing is fail-closed where physics fixes the bound: the Nyquist
   ...
   are advisory only: the ns-scale implosion-asymmetry evolution range 1 MHz–10 GHz and the sub-ns bunch-arrival precision of heavy-ion and pulsed-electron drivers (R. O. Bangerter et al., J. Fusion Energy 32 (2013) 549).
5. The reference plan mirrors canonical practice with synthetic
   declarations: a bunch-arrival event train (event relative against the shot fiducial), streaked-radiography trajectory and shot-outcome sets (noncyclic), an implosion-asymmetry mode set (derived cyclic against the facility clock), and the model-owned synthetic oscillator (simulation clock). Every applicable candidate must be
