...
SCPN ICF Beam Core — ADR 0002: device configuration model
-->
...
configurations this repository owns (`ion_beam_icf`,
`pulsed_electron_beam_icf`). The claim boundary and repository-level
`evidence_maturity` semantics follow the family pilot.

## Decision

1. The package `scpn_icf_beam_core` implements the device configuration
   model as frozen, strictly typed value objects: the particle-beam
   driver (species class, total energy, pulse duration, per-particle
   kinetic energy) and the target declaration (pellet radius).
...
3. Hard class invariant: the driver species must match the identifier —
   ``ion`` for `ion_beam_icf`, ``electron`` for
   `pulsed_electron_beam_icf`.
4. Derived quantity: the beam power ``P = E / tau``. Advisory finding,
   reported by `consistency_report()` and never clamped: an ion-class
   per-particle kinetic energy outside the documented heavy-ion-fusion
   driver window ``[1, 10] GeV`` (R. O. Bangerter, A. Faltens,
   P. A. Seidl, Rev. Accel. Sci. Technol. 6 (2013) 85); the window is
   not applied to the electron class.
...
- Later lanes (driver/burn diagnostic semantics with the bang-time
