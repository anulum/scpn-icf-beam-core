...
# SCPN ICF Beam Core — diagnostic and clock semantics tests
...
from scpn_icf_beam_core.errors import DiagnosticPlanError
...
EVENT_BINDINGS = {
    ...
    "event_reference": "synthetic bunch fiducial marker",
    "provenance": "synthetic fixture",
...
REFERENCE_FRAMES = (
    ReferenceFrame(
        ...
        description="accelerator beamline frame to target chamber centre",
    ),
    ReferenceFrame(
...
SIGNALS_CH_BUNCH_TIMING_TRAIN = (
    SignalDeclaration(
        identifier="sig_bunch_charge",
        quantity="charge",
        unit="C",
        role=SignalRole.AMPLITUDE,
        description="synthetic bunch charge",
    ),
    SignalDeclaration(
        identifier="sig_bunch_cycle",
...
def clock_shot() -> ClockModel:
    ...
    return ClockModel(
        ...
        resolution_s=1.0e-11,
        uncertainty_s=1.0e-11,
    )
...
def channel_event_train() -> DiagnosticChannelPlan:
    """Build the synthetic bunch-arrival event channel."""
    return DiagnosticChannelPlan(
        identifier="ch_bunch_timing_train",
...
def synthetic_plan() -> DiagnosticPlan:
    ...
    return DiagnosticPlan(
        identifier="icf_beam_reference_plan",
...
def test_candidate_properties_follow_class_tables() -> None:
    ...
    mode = by_id["inertial.resolved_asymmetry_mode"]
    assert SemanticCarrier.COMPLEX_MODE in mode.admissible_carriers
    assert "observation_operator" in mode.required_evidence
    timing = by_id["inertial.driver_timing"]
    assert SemanticCarrier.EVENT_CYCLE in timing.admissible_carriers
    assert "timing_uncertainty" in timing.required_evidence
...
def test_candidate_rejects_unsorted_configurations() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        CandidateProfile(
            candidate_id="a.b",
            phenomenon="x",
            configurations=("pulsed_electron_beam_icf", "ion_beam_icf"),
            observability_class=ObservabilityClass.NONCYCLIC_FEATURE,
        )
...
def test_channel_rejects_unsorted_or_duplicate_signals() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(signals=tuple(reversed(SIGNALS_CH_ASYMMETRY_MODE_SET)))
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(
            signals=(*SIGNALS_CH_ASYMMETRY_MODE_SET, SIGNALS_CH_ASYMMETRY_MODE_SET[-1])
        )
...
def test_non_event_channel_rejects_timing_marker() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="only event-relative"):
        _derived(signals=(*SIGNALS_CH_ASYMMETRY_MODE_SET, marker))
...
def test_transformation_rejects_self_mapping() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="to itself"):
        _transformation(target_identifier="frm_beamline")
...
def test_plan_rejects_unsorted_transformations() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="must be sorted"):
        _plan_with(
            frames=tuple(
                sorted(
                    (*plan.frames, _extra_frame("frm_zz_extra")),
                    key=lambda frame: frame.identifier,
                )
            ),
            frame_transformations=(
                _transformation(
                    source_identifier="frm_chamber",
    ...
    with pytest.raises(DiagnosticPlanError, match="duplicate transformation pair"):
        _plan_with(
            frame_transformations=(
                _transformation(),
                _transformation(
                    source_identifier="frm_chamber", target_identifier="frm_beamline"
                ),
            )
        )
...
def test_plan_rejects_transformation_to_undeclared_frame() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="is not declared"):
        _plan_with(
            frame_transformations=tuple(
                sorted(
                    (
                        *synthetic_plan().frame_transformations,
                        _transformation(target_identifier="frm_zz_ghost"),
                    ),
                    key=lambda item: (item.source_identifier, item.target_identifier),
                )
            )
        )
...
def test_plan_rejects_inadmissible_transformation_kind() -> None:
    ...
    wrong = _transformation(
        kind=TransformationKind.PROJECTION, equilibrium_dependent=False
    )
    with pytest.raises(DiagnosticPlanError, match="not admissible"):
        _plan_with(
            frame_transformations=tuple(
                ...
                == ("frm_beamline", "frm_chamber")
                else item
                for item in synthetic_plan().frame_transformations
            )
        )
...
def test_single_frame_plan_needs_no_transformation() -> None:
    ...
    plan = synthetic_plan()
    referenced = {
        channel.evidence_bindings.get("coordinate_frame") for channel in plan.channels
    } - {None}
    kept = (
        tuple(frame for frame in plan.frames if frame.identifier in referenced)[:1]
        or plan.frames[:1]
    )
    channels = tuple(
        dataclasses.replace(
            channel,
            evidence_bindings={
                **channel.evidence_bindings,
                **(
                    {"coordinate_frame": kept[0].identifier}
                    if "coordinate_frame" in channel.evidence_bindings
                    else {}
                ),
            },
        )
    ...
    variant = _plan_with(frames=kept, frame_transformations=(), channels=channels)
...
def test_report_flags_cyclic_array_without_amplitude_signal() -> None:
    ...
    channel = _derived(
        signals=tuple(
            signal
            for signal in SIGNALS_CH_ASYMMETRY_MODE_SET
...
def test_channel_rejects_nyquist_violation() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="Nyquist"):
        _derived(sample_rate_hz=5.0e08)
...
def test_plan_accepts_explicit_deferral() -> None:
    ...
    plan = DiagnosticPlan(
        identifier="icf_beam_partial_plan",
        binding=CATALOGUE_BINDING,
...
def test_report_flags_mhd_band_outside_typical_range() -> None:
    ...
    channel = _derived(sample_rate_hz=1.0e12, max_signal_frequency_hz=5.0e11)
    plan = DiagnosticPlan(
        identifier="icf_beam_reference_plan",
        binding=CATALOGUE_BINDING,
        clocks=(clock_facility(), clock_shot(), clock_simulation()),
    ...
    findings = plan.consistency_report()
    assert len(findings) == 1
    assert "Bangerter" in findings[0].message
...
def test_report_flags_clock_coarser_than_sampling() -> None:
    ...
    plan = DiagnosticPlan(
        ...
        deferrals=(
            DeferredCandidate(
                candidate_id="inertial.driver_timing",
                reason="driver timing train deferred pending event fixtures",
            ),
        ),
    )
    findings = plan.consistency_report()
...
def test_parser_accepts_integer_numbers() -> None:
    ...
    record = synthetic_plan().to_record()
    record["channels"][3]["sample_rate_hz"] = 1000
    assert plan_from_record(record).channels[3].sample_rate_hz == 1000.0
...
def test_bytes_parser_rejects_nan_literal() -> None:
    ...
    record = synthetic_plan().to_record()
    text = json.dumps(record).replace("5e-10", "NaN")
...
def test_plan_rejects_unknown_frame_reference() -> None:
    ...
    plan = synthetic_plan()
    kept = tuple(frame for frame in plan.frames if frame.identifier != "frm_chamber")
...
def test_report_flags_window_beyond_device_ceiling() -> None:
    ...
    channel = _derived(acquisition_duration_s=9.999999999999999e-06)
    plan = synthetic_plan()
    plan = DiagnosticPlan(
        identifier=plan.identifier,
        binding=plan.binding,
