...
# SCPN ICF Beam Core — diagnostic and clock semantics
...
identities for the particle-beam inertial-confinement configurations this
repository owns, aligned with the SCPN Phase Orchestrator
observability-profile catalogue release
...
from scpn_icf_beam_core.configuration import ConsistencyFinding
...
ASYMMETRY_BAND_HZ: Final = (1.0e06, 1.0e10)
DRIVER_TIMING_CEILING_S: Final = 1.0e-09

SHOT_DURATION_CEILING_S: Final = 1.0e-06
...
_CLOCK_KEY: Final[dict[ObservabilityClass, str]] = {
    ...
    ObservabilityClass.NONCYCLIC_FEATURE: "clock_epoch",
...
_COMPATIBLE_CLOCK_KINDS: Final[dict[ObservabilityClass, frozenset[ClockKind]]] = {
    ...
    ObservabilityClass.NONCYCLIC_FEATURE: frozenset(
        {ClockKind.FACILITY_MONOTONIC, ClockKind.SHOT_EVENT_EPOCH}
    ),
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-ICF-BEAM-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    CandidateProfile(
        ...
        phenomenon=("laser, beam, projectile, or igniter timing within a shot"),
        configurations=("ion_beam_icf", "pulsed_electron_beam_icf"),
    ...
    CandidateProfile(
        ...
        phenomenon="resolved nonzero-order implosion asymmetry",
        configurations=("ion_beam_icf", "pulsed_electron_beam_icf"),
        observability_class=ObservabilityClass.DERIVED_CYCLIC,
    ),
    CandidateProfile(
        candidate_id="inertial.shot_outcome",
        phenomenon=("yield, ion temperature, areal density, and burn outcome"),
        configurations=("ion_beam_icf", "pulsed_electron_beam_icf"),
        observability_class=ObservabilityClass.NONCYCLIC_FEATURE,
    ),
    CandidateProfile(
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-ICF-BEAM-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def _validate_transformations(
        ...
        """
        keys = tuple(
            (transformation.source_identifier, transformation.target_identifier)
            for transformation in self.frame_transformations
        )
        if tuple(sorted(keys)) != keys:
            raise DiagnosticPlanError("plan.frame_transformations: must be sorted")
        seen: set[frozenset[str]] = set()
        adjacency: dict[str, set[str]] = {
            identifier: set() for identifier in frames_by_id
        }
        for transformation in self.frame_transformations:
            pair = frozenset(
                {transformation.source_identifier, transformation.target_identifier}
            ...
            if pair in seen:
                raise DiagnosticPlanError(
                    "plan.frame_transformations: duplicate transformation pair "
                    f"{sorted(pair)!r}"
                )
            seen.add(pair)
            kinds: list[FrameKind] = []
            for identifier in sorted(pair):
                frame = frames_by_id.get(identifier)
                if frame is None:
                    raise DiagnosticPlanError(
                        "plan.frame_transformations: frame "
                        f"{identifier!r} is not declared"
                    )
                kinds.append(frame.kind)
            admissible = _ADMISSIBLE_TRANSFORMATIONS.get(frozenset(kinds))
            if admissible is None or admissible is not transformation.kind:
                raise DiagnosticPlanError(
                    "plan.frame_transformations: kind "
                    f"{transformation.kind.value!r} is not admissible between "
                    f"{kinds[0].value!r} and {kinds[1].value!r}"
                )
            adjacency[transformation.source_identifier].add(
                transformation.target_identifier
            )
            adjacency[transformation.target_identifier].add(
                transformation.source_identifier
            )
        if len(frames_by_id) >= 2:
            start = next(iter(frames_by_id))
            reached = {start}
            frontier = [start]
            while frontier:
            ...
            if reached != set(frames_by_id):
                raise DiagnosticPlanError(
                    "plan.frame_transformations: declared frames are not connected; "
                    f"unreachable={sorted(set(frames_by_id) - reached)!r}"
                )
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented beam-ICF diagnostic practice; empty
            when every declaration sits in its typical band. Findings
            are advisory instruments, not machine claims.
        """
        findings: list[ConsistencyFinding] = []
        clocks_by_id = {clock.identifier: clock for clock in self.clocks}
        low, high = ASYMMETRY_BAND_HZ
        for channel in self.channels:
            observability_class = channel.observability_class
            if channel.candidate_id == "inertial.resolved_asymmetry_mode" and not (
                low <= channel.max_signal_frequency_hz <= high
            ):
                findings.append(
                    ConsistencyFinding(
                        field=f"channels[{channel.identifier}].max_signal_frequency_hz",
                        message=(
                            "declared asymmetry-mode band "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the ns-scale implosion "
                            "asymmetry evolution range 1 MHz-10 GHz "
                            "(Bangerter 2013)"
                        ),
                    )
                )
            if (
                observability_class is ObservabilityClass.EVENT_RELATIVE
                and channel.timing_uncertainty_s is not None
                and channel.timing_uncertainty_s > DRIVER_TIMING_CEILING_S
            ):
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            " is coarser than the sub-ns "
                            "bunch-arrival precision of beam drivers "
                            "(Bangerter 2013)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the beam-ICF shot scale of up to ~1 us "
                            "(Bangerter 2013)"
                        ),
                    )
                )
